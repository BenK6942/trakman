...
export class LiveSplitsRepository extends Repository {
  ...
  async addOrUpdate(mapUid: string, login: string, finishTime: number, personBest: number): Promise<void> {
    try {
	...
	DO UPDATE SET 
      ...
      const values: any[] = [mapId, mapUid, playerId, login, finishTime, personBest]
